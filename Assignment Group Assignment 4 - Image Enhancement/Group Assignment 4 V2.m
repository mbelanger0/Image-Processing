...
figure; imshow(J); title('Preliminary Restoration with Blind Deconvolve');
figure; imshow(P,[]);
...
title('Gaussian Restoration with Lucy Deconvolve');
subplot(223);imshow(J2);
title('Restoration using Original PSF with Lucy Deconvolve');
%% 
% Some of the pros of using predesigned functions are:
%% 
% * They are simple/quick to use
% * They are accessible to the wider population (people with little image processing 
% knowledge) 
% * They are often easy to find example code
% * They reduce sources of error because the complex math is already done for 
% the user
%% 
% Some of the cons of using predesigned functions are:
%% 
% * It is a black box of code 
% * There are limited options for customization and optimization
% * It encourages copy and pasting code without fully understanding it
% * They frequently produces unexpected _and unexplainable_ results 
%% 
...
% of concepts in machine learning and optimization. Based on research by our team, 
% a few ways to find a better kernel are detailed below:
% 
...
% the gradient (blur) along the edges will be the same for every point on the 
...
figure()
imshow(carter)
title("Carter Original Image")
imhist(carter);
title("Original Carter Histogram")
normal_carter = histeq(carter);
imhist(normal_carter);
title("Normalized Carter Histogram")
imshow(normal_carter)
title("Normalized Carter Image")
beach = imread("beach.jpg");
figure()
imshow(beach)
title("Beach Original Image")
imhist(carter);
title("Original Beach Histogram")
normal_carter = histeq(beach);
imhist(normal_carter);
title("Normalized Beach Histogram")
imshow(normal_carter)
title("Normalized Beach Image")
%% 
...
% Upon using MATLAB's built-in histeq() function, the equalization was immedietly 
% apparent because of the increased saturation and overall brightness of the image. 
% This caused the colors to appear much more vivid that the original image. This 
% was especially apprent for Carter shirt and face, while the background was only 
% impacted mildly.
% 
% 
% 
...
% colors in the image, but in a different way than it had changed the image of 
% Carter. This image came out much darker than the input image, which although 
% muted, was fairly bright all around. The saturation-like effect seen in the 
% Carter image didn't fully appear in the image of the beach.
% 
% 
% 
...
% in the image was lost. Both the Gaussian and Butterworth images looked generally 
% blurred, but at this low of a frequency, small differences were noticeable. 
